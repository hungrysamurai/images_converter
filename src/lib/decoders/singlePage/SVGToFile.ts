...
export const SVGToFile = async (
 ...
 const svgEl = new DOMParser().parseFromString(svgText, MIMETypes.SVG).documentElement as unknown as SVGSVGElement;
 ...
 if (!currentWidth || !currentHeight || typeof currentHeight !== 'number' || typeof currentWidth !== 'number') {
 ...
 if (resize && (targetWidth || targetHeight)) {
  ...
  // Scale SVG
  const targetDimensions = getScaledSVGDimensions(currentHeight, currentWidth, units, targetWidth, targetHeight);
 ...
 return new Promise((resolve, reject) => {
  ...
  img.onload = async () => {
   try {
    ...
    if (activeTargetFormatName !== OutputFileFormatsNames.PNG && activeTargetFormatName !== OutputFileFormatsNames.TIFF && activeTargetFormatName !== OutputFileFormatsNames.WEBP) {
     ctx.fillStyle = 'white'
